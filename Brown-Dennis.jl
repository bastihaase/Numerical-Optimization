function Brown_Dennis(f,grad,J,r,x_0,n,m;maxIter=100,atol=10.0^(-8))
	B=zeros(n,n,m)
	for i=1:m
	...
	Jc=J(xc)
	df=Jc'*rc
	performance=[0,m*2+1,1,0,0] #eval f, eval grad J, matrix matrix, matrix vector,matrix fact 
	while(norm(df)>atol && iter<=maxIter)
		...
		#update everything
		for j=1:m
			B[:,:,j]=B[:,:,j]+(J_new[j,:]'-Jc[j,:]'-B[:,:,j]*(xc_new-xc))*(xc_new-xc)'/norm(xc_new-xc)^2
		end
		performance+=m*[0,1,2,2,0]+[0,0,0,0,1]
		iter=iter+1
		Jc=J_new
		xc=xc_new
	end
